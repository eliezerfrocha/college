...
import com.lista.exercicios.atividades.model.MatrizDTO;
import com.lista.exercicios.atividades.model.SomaLinhasDTO;
...
public class MatematicaController {
    ...
    public ResponseEntity<?> somaLinhas(@RequestBody MatrizDTO matrizDTO) {
        List<List<Integer>> matriz = matrizDTO.getMatriz();

        if (matriz == null || matriz.isEmpty()) {
            return ResponseEntity.badRequest()
                    .body(new ErrorDTO("400", "A matriz não pode estar vazia."));
        }

        int tamanhoEsperado = matriz.get(0).size();
        for (List<Integer> linha : matriz) {
            if (linha == null || linha.size() != tamanhoEsperado) {
                return ResponseEntity.badRequest()
                        .body(new ErrorDTO("400", "Todas as linhas devem ter o mesmo tamanho."));
            }
            if (linha.stream().anyMatch(Objects::isNull)) {
                return ResponseEntity.badRequest()
                        .body(new ErrorDTO("400", "A matriz contém valores inválidos (null)."));
            }
        }
        ...
        return ResponseEntity.ok(new SomaLinhasDTO(somas));
    }
}
