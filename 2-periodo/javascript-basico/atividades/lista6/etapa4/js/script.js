...
(function(){
    ...
    function renderBoard(deckArr){
        ...
        // Calcular tamanho do container baseado no número de cartas
        const containerWidth = pares <= 4 ? '800px' : '1100px';
        document.querySelector('.container').style.maxWidth = containerWidth;

        deckArr.forEach((num, idx) => {
            const card = document.createElement('div');
            card.className = 'card'; // começa mostrando a face
            ...
            // Não adiciona classe flipped aqui - começa mostrando a face
    ...
    btnIniciar.addEventListener('click', ()=>{
        ...
        renderBoard(deck);
        showMessage('Memorize as cartas!');
        
        const cards = document.querySelectorAll('.card');
        // Começa mostrando as faces (cartas)
        cards.forEach(card => card.classList.remove('flipped'));
        
        // Após 3 segundos, vira todas para o verso e inicia
        setTimeout(() => {
